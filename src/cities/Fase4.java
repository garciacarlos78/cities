...
public class Fase4 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		// Char arrays for the cities
		char[][] charCities = new char[arrayCiutats.length][];
		
		// Initialize the array of arrays
		for (int i=0;i<charCities.length;i++) {
			charCities[i] = new char[arrayCiutats[i].length()];
			for (int j=0; j < charCities[i].length; j++) {
				charCities[i][j] = arrayCiutats[i].charAt(j);
			}
		}
		
		// StringBuilder per crear string amb noms invertits
		StringBuilder sb = new StringBuilder("Noms invertits: ");
		
		// Show new array names with inverted order
		for (int i=0; i < charCities.length; i++) {
			for (int j=charCities[i].length-1; j>=0; j--) {
				sb.append(charCities[i][j]);
			}
			if (i!= charCities.length-1) sb.append(", ");
		}
		
		// Show the name of the cities inverted
